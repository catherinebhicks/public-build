...
piece can be tested on its own, and each can be swapped — every provider sits
behind an adapter, so replacing the STT vendor doesn't mean rebuilding the
product.

## The portfolio reason, said out loud
...
I'd rather be straight about that than pretend the architecture was chosen on
purely technical merit. It's a real input. It's also the kind of thing that
...
Polish can come later — the seams are where the upgrade would go.

## No voice cloning

Worth stating since it's the obvious next idea: **no voice cloning in V1.**

It adds consent, privacy, security, and abuse problems — you're accepting
